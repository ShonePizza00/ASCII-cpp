...
namespace plotter {
	...
	void DemoRunner::CompareFillAlgorithms()
	{
		...
		const auto floodfill_begin = std::chrono::steady_clock::now();
		plotter1.FloodFill(10, 10, 'F');
		const auto floodfill_end = std::chrono::steady_clock::now();
		
		const auto scanline_begin = std::chrono::steady_clock::now();
		plotter2.ScanlineFill(10, 10, 'S');
		const auto scanline_end = std::chrono::steady_clock::now();

		const auto floodfill_time = std::chrono::duration_cast<std::chrono::microseconds>(floodfill_end-floodfill_begin).count();
		const auto scanline_time = std::chrono::duration_cast<std::chrono::microseconds>(scanline_end - scanline_begin).count();
